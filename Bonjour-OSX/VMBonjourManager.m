...
@interface VMBonjourManager (HelperMethods)

- (NSString *)typeStringFromTypeName:(NSString *)name transportProtocol:(NSString *)transportProtocol;

@end

@implementation VMBonjourManager

+ (id)sharedManager {
    static VMBonjourManager *sharedMyManager = nil;
...
- (void)publish:(NSString *)serviceType transportProtocol:(NSString *)transportProtocol port:(uint16_t)port delegate:(id<VMBonjourPublishDelegate>)delegate {
    ...
    if (service) {
        ...
        if (delegate) {
            [delegate registerFailed:[NSError errorWithDomain:kVMBonjourPublishDomain
                                                         code:kVMBonjourPublishErrorRegisterFailed
                                                     userInfo:nil]];
        }
    }
}
...
- (void)connectToService:(NSNetService *)service delegate:(id<NSStreamDelegate>)delegate error:(NSError **)error {
    ...
    if (inStream && outStream)
    {
        ...
        *error = [NSError errorWithDomain:kVMBonjourConnectDomain
                                     code:kVMBonjourConnectErrorConnectFailed
                                 userInfo:nil];
    }
}
...
- (void)netServiceDidPublish:(NSNetService *)sender {
    ...
    id<VMBonjourPublishDelegate> delegate = [self.publishDelegates objectForKey:type];
    if (delegate) {
...
- (void)netService:(NSNetService *)sender didNotPublish:(NSDictionary *)errorDict {
    ...
    id<VMBonjourPublishDelegate> delegate = [self.publishDelegates objectForKey:type];
    if (delegate) {
        [delegate publishFailed:[NSError errorWithDomain:kVMBonjourPublishDomain
                                                    code:kVMBonjourPublishErrorPublishFailed
                                                userInfo:errorDict]];
    }
}
...
- (void)netServiceBrowser:(NSNetServiceBrowser *)aNetServiceBrowser didNotSearch:(NSDictionary *)errorDict {
    if (self.serviceBrowserDelegate) {
        [self.serviceBrowserDelegate searchFailed:[NSError errorWithDomain:kVMBonjourBrowseDomain
                                                                      code:kVMBonjourBrowseErrorBrowseFailed
                                                                  userInfo:errorDict]];
    }
}
